...
| HM1240401C | `a[href]` must have link text; hidden/template/slot elements are skipped; image links require non-empty `img alt`; SVG role img needs accessible name. | Mirror visible link text to `title`; copy `title` to `aria-label` for icon-only links; aggressive fallback from href; linked images get inferred alt. |
