...
- 同站 CSS 可由 OB 轉為 inline `<style>`，展開同站 `@import`，並只將 `font-size` 的絕對單位轉為 `rem` 以支援 Freego AA 檢測
...
- CSS inline repair 預設處理同站且位於 WordPress 根目錄內的 local CSS，且只轉換 `font-size`，不會改寫 layout 尺寸；可用 `freego_wp_inline_css_repair_enabled` 與 `freego_wp_inline_css_repair_allowed_paths` filters 調整路徑範圍
- 完整 AAA 仍需要人工語意稽核
