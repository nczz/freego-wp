...
- 同站 CSS 可由 OB 轉為 inline `<style>`，展開同站 `@import`，並轉換 `font-size`、`max-width`、`line-height` 的絕對單位以支援 Freego 檢測
...
OB 修補與 Freego checker predicate 的對照文件位於：
...
- CSS inline repair 預設處理同站且位於 WordPress 根目錄內的 local CSS，且只轉換 `font-size`、`max-width`、`line-height` 的絕對尺寸單位；可用 `freego_wp_inline_css_repair_enabled` 與 `freego_wp_inline_css_repair_allowed_paths` filters 調整路徑範圍
- 完整 AAA 仍需要人工語意稽核
