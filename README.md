...
- 同站 allowlist CSS 可由 OB 轉為 inline `<style>`，並只將 `font-size` 的 `px` 轉為 `rem` 以支援 Freego AA 檢測
...
- CSS inline repair 預設只處理同站 `wp-content/themes/` 與 `wp-content/uploads/flipbook/` 下的 CSS，且只轉換 `font-size`，不會改寫 layout 尺寸；可用 `freego_wp_inline_css_repair_enabled` 與 `freego_wp_inline_css_repair_allowed_paths` filters 調整
- 完整 AAA 仍需要人工語意稽核
